...
//  
 ...
 const titleName =  content && content.name && content.name !== '' ? content.name : content && content.title && content.title !== '' ?  content.title : '';
    ...
    const id = params.movieid || params.moviesid || '';
    const _media_type = params && params.mediatype &&  params.mediatype !== '' ? params.mediatype.toLowerCase() : '';
    const API_KEY = process.env.REACT_APP_NOT_SECRET_CODE;
  
 const fetchData = async () =>{
        try{
        const { data } = await axios.get(`https://api.themoviedb.org/3/${_media_type}/${id}?language=en-US`, {
      headers: {
        Authorization: `Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJhNDkwOWFhZDJhZTY0ZThiMzJkMDgxMzY5YmQyYzI1YyIsIm5iZiI6MTc1MTYxNzE3NC4wODMwMDAyLCJzdWIiOiI2ODY3OGU5NmJhMDAxNTIwNGQ5YmM4OGIiLCJzY29wZXMiOlsiYXBpX3JlYWQiXSwidmVyc2lvbiI6MX0.qSmnX3x87GS4GN1HA2kw6YD5q3c8HBUplppbmYsbjsM`
      }
    });
          setContent(data);
    ...
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/${_media_type}/${id}/videos?language=en-US`,
      {
        headers: {
          Authorization: `Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJhNDkwOWFhZDJhZTY0ZThiMzJkMDgxMzY5YmQyYzI1YyIsIm5iZiI6MTc1MTYxNzE3NC4wODMwMDAyLCJzdWIiOiI2ODY3OGU5NmJhMDAxNTIwNGQ5YmM4OGIiLCJzY29wZXMiOlsiYXBpX3JlYWQiXSwidmVyc2lvbiI6MX0.qSmnX3x87GS4GN1HA2kw6YD5q3c8HBUplppbmYsbjsM`
        }
    ...
    const creditsFetch = async ()=>{
        try{
           const { data } = await axios.get(
      `https://api.themoviedb.org/3/${_media_type}/${id}/credits?language=en-US`,
      {
        headers: {
          Authorization: `Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJhNDkwOWFhZDJhZTY0ZThiMzJkMDgxMzY5YmQyYzI1YyIsIm5iZiI6MTc1MTYxNzE3NC4wODMwMDAyLCJzdWIiOiI2ODY3OGU5NmJhMDAxNTIwNGQ5YmM4OGIiLCJzY29wZXMiOlsiYXBpX3JlYWQiXSwidmVyc2lvbiI6MX0.qSmnX3x87GS4GN1HA2kw6YD5q3c8HBUplppbmYsbjsM`
        }
